...
exports.resolvers = {
	Query:{
        ...
        getCartById: async (_, args) => {
            ...
            const cartData= await fsPromises.readFile(cartsFilePath, { encoding: 'utf-8' })
    ...
    Mutation:{
        ...
        const { productId, productName, productPrice, productType, productStatus } = args.input
          
         if (productName?.length === 0) return new GraphQLError('Name must be at least 1 character long')
         ...
         const filePath = path.join(productsDirectory,`${newProduct.productId}.json`)
        ...
        addProductsToCart: async (_, args) => {
          ...
          if (!productsExists) return new GraphQLError('This product does not exist')

          if(productsExists){
          const productData= await fsPromises.readFile(productsFilePath,{ encoding: 'utf-8' })
        ...
        /* productsTotalPrice:async (_, args) => {
          const {cartId} = args;
          const cartsFilePath = path.join(cartsDirectory,`${cartId}.json`)
 ...
 */
        ...
        removeProductsFromCart:async (_, args) => {
         ...
         const removeItem = (cartItems, item)=>{
          const findIndex= cartItems.findIndex((cartItems) => cartItems.productId === item)  
          if(findIndex > -1){
